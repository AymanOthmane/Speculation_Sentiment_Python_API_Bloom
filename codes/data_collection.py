...
class yf_Data:
    ...
    @staticmethod
    def get_ETF_data(feature = "Close"):
        """
        ...
        """
        df_ETF_date =  'SSO QLD DDM QID SDS DXD'
        data = yf.download(df_ETF_date, start="2008-01-01", end="2023-04-30")[feature]
        return data
    ...
    def get_yf_data(symbol,start_date,end_date, field = 'Close'):
        ...
        if "YFPricesMissingError" in data or data.array.size == 0:
            ...
            data = yf_Data.get_yf_data(symbol, start_date, end_date, field = field)
        if isinstance(data,float):
            return data
        if len(data)>1: 
            return round(float(data.values[0]),2)
        else :
            return np.round(float(data.values),2)
        

    def get_yf_datas(symbol,start_date,end_date,feature="Close"):
        """
        Get historical daily close data for the leveraged ETFs mentionned in the article
        for the dates mentioned in the article from Yahoo Finance.
        """
        data = yf.download(symbol, start=start_date, end=end_date)[feature]
        return data
