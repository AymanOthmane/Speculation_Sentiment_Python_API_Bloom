...
class Trading_Strat:
    ...
    def fn_GRU(self):
        ...
        print(type(y_train))
        print(type(y_pred))
        y_pred = y_train.tolist() + y_pred.flatten().tolist()
        data['Predicted_TurningPoint'] = 0
        data.iloc[-len(y_pred):, data.columns.get_loc('Predicted_TurningPoint')] = y_pred
    ...
    def calculate_metrics(self,frequence):
        self.df_Backtest['Daily_Return'] = self.df_Backtest['Cumulative Return'].pct_change().dropna()

        # Overall performance
        overall_performance = self.df_Backtest['Cumulative Return'].iloc[-1] / self.df_Backtest['Cumulative Return'].iloc[0] - 1

        # Annualized performance
        n_years = (self.df_Backtest.index[-1] - self.df_Backtest.index[0]).days / 365.25
        annualized_performance = (1 + overall_performance) ** (1 / n_years) - 1

        # Volatility
        monthly_volatility = self.df_Backtest['Daily_Return'].resample("ME").mean().dropna().std()
        annualized_volatility = monthly_volatility * np.sqrt(12)
        vol = monthly_volatility
        if frequence == "D":
            daily_volatility = self.df_Backtest['Daily_Return'].std()
        ...
        # Sharpe ratio
        risk_free_rate = 0.01 
        sharpe_ratio = (self.df_Backtest['Daily_Return'].mean() - risk_free_rate / 252) / vol

        # Maximum drawdown
        roll_max = self.df_Backtest['Cumulative Return'].cummax()
        daily_drawdown = self.df_Backtest['Cumulative Return'] / roll_max - 1.0
        max_drawdown = daily_drawdown.cummin().min()

        # Historical VaR (Value at Risk)
        var = np.percentile(self.df_Backtest['Daily_Return'].dropna(), 5)
